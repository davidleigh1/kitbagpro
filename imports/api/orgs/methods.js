...
Meteor.methods({
	addOrg: function(orgObj){
		console.log('FN: Meteor.methods.addOrg()(>>'+thisCollectionName+'<<)',orgObj);
		...
		if ( !globalIsThisObjectUnique(orgObj._id, thisCollectionName) ){
		...
		var dbNewORG = kb.collections[thisCollectionName].insert(orgObj);
		/*
	...
	updateOrgField: function(org_id, field, newValue){
		...
		// kb.collections[thisCollectionName].update( org_id , { $set: { field : newValue } });	
		setDocField(thisCollectionName, org_id, field, newValue);
	},
	// deleteOrg: function(org_id, userId){
	// 	// var res = MyCollections["Orgs"].findOne(id);
	// 	var res = kb.collections[thisCollectionName].findOne(org_id);
	...
	// 		kb.collections[thisCollectionName].remove(org_id);
	// 		serverlog({
	// 			actor: userId,
	// 			subject: org_id,
	// 			object: "Org",
	...
	// 	}else{
	// 		kb.collections[thisCollectionName].update(org_id, { $set: {private: private}});
	// 	}
	// },
 ...
 //    // var res = MyCollections["Kitbags"].findOne(id);
	...
	// 	}else{
	// 		kb.collections[thisCollectionName].update(org_id, { $set: {status: newStatus}});
	// 		console.log("status set: ",kb.collections[thisCollectionName].findOne(org_id));
	// 	}
	// }
});